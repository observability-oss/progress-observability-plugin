---
description: Add Progress Observability instrumentation to an existing agent (Python, TypeScript, or .NET) and verify traces arrive.
...
works with no LLM credentials), then verify over MCP with `list_observations`
that spans arrived and report instrumentation depth — not just arrival.
